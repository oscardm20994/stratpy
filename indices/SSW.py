...
class SSW_class:
	...
	def get_SSW_dates(self, ZMZW_cube, thresh = 0):
		...
		SSW_month, SSW_year, SSW_day = SSW_counter(ZMZW_cube, self.years[0], self.years[-1], thresh)
	...
	def SSW_rate_Er(self, resample_interval):
		...
		self.get_SSWs_timeseries()
		## resample SSW time series and calculate set of rates
		rates = np.empty(0)
		for i in range(10000):
			rates = np.append(rates, np.mean(np.random.choice(self.SSW_timeseries, resample_interval, replace = False)))
	...
	def get_SSW_rate_Er(self, resample_interval):
